# Create Branding for Store

## Overview
Create a branding configuration for customizable pages at the store level.

## Prerequisites
- An active merchant account.
- Valid `merchantId` and `storeId`.
- Authentication via `API-KEY`, `API-SECRET` and `MERCHANT-ID` headers.
...
### Possible Errors
N/A. The provided JSON did not have any error examples.

## Notes
- The color parameters should be valid hexadecimal color codes.
- Ensure the image URLs provided for `logoUrl`, `iconUrl`, `primaryCoverImage`, and `secondaryCoverImage` are publicly accessible.
