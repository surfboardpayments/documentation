...
- Ensure you have a valid Order ID to fetch the order details.
...
| `data.controlFunctions.receipt.promotionsToDisplay` | array | Promotions to display on receipt |
...
| `data.controlFunctions.readTags` | string | This field denotes the type of NFC reading allowed on the terminal for the order. The possible values are, |
...
| `data.controlFunctions.online.payButtonType` | string | UI pay button label/type used on the payment page. |
...
| `data.controlFunctions.online.recurring.subscriptionAmountType` | string | Determines the type of the subscription amount for order. |
...
```
**Description:** This error occurs when invalid or missing authentication credentials are provided. Ensure that the API-KEY, API-SECRET, and MERCHANT-ID headers are correctly set with valid values.

## Notes
- The `orderId` parameter in the URL path is case-sensitive.
- Ensure that the `orderId` you are using is a valid and existing order identifier.
- The currency is represented as a numeric ISO 4217 code (string).
- A negative value for `quantity` indicates a refund.
- Several fields are marked as "comingSoon" and are subject to future updates.
---